...
class BobRossClient {
  ...
  getSrcset(hash, options) {
    ...
    invariant(
      options.resize,
      `The getSrcset options param requires a resize key.`
    );
    var size_match = options.resize.match(/(\d+)x(\d+)(.*)/);
    invariant(
      size_match,
      `Invalid resize value: ${options.resize}. Must be in the form "100x150".`
    );
    ...
    return [
      [
        this.getUrl(
          hash,
          _.extend({}, options, {
            resize: width * 2 + "x" + height * 2 + style
          })
        ),
        "2x"
      ].join(" "),
      [
        this.getUrl(
          hash,
          _.extend({}, options, {
            resize: width * 3 + "x" + height * 3 + style
          })
        ),
        "3x"
      ].join(" ")
  ...
  isImage(url, timeoutT = 5000) {
    return new Promise((resolve, reject) => {
      const timeout = timeoutT;
      let timer = null;
      const img = new Image();
